...
const context=vm.createContext({Worker,setTimeout,clearTimeout,console});vm.runInContext(fs.readFileSync('chess.js','utf8')+fs.readFileSync('engine.js','utf8')+';globalThis.api={Chess,StockfishOpponent,BOT_LEVELS};',context);
const {Chess,StockfishOpponent,BOT_LEVELS}=context.api;
(async()=>{
